...
| L3 触发计数排除文档派生原子：`markL1ExtractionComplete` 新增 `personaVisibleExtracted` 参数，文档派生 stored 数不计入 `memories_since_last_persona`（L2 场景块**照常**消费文档派生原子——2026-08-18 再修订，曾短暂全排除后回退 L2 半边） | 批量导文档不催更画像（避免"内容其实没变"的空转重生成）；persona 内容只从场景块生成，文档知识经 L2 自然纳入；回归测试 `__tests__/pipeline/l2-l3-doc-semantics.test.ts` |
