...
| L2/L3 都排除文档派生原子（2026-08-19 定稿）：`createL2Runner` 查询后过滤 source_kind=document（纯文档批次仍推进游标）；`markL1ExtractionComplete` 的 `personaVisibleExtracted` 让文档派生 stored 数不计入 `memories_since_last_persona`（演进：08-18 全排除 → 08-18 再修订 L2 恢复消费 → 08-19 定稿 L2 也排除） | 场景画像只由对话行为塑造，批量导文档不催更不污染 L2/L3；文档知识的消费面是召回与溯源，不是画像；回归测试 `__tests__/pipeline/l2-l3-doc-semantics.test.ts` |
| 文档模式类型限定收紧为 work_*：提炼 prompt 枚举去掉 instruction，提取后过滤加拦 instruction（与 persona/episodic 同列丢弃） | instruction 是对话链路的"用户指令"类型，不从文档提取；文档中的行为规则归 work_method（constraint/anti_pattern）；回归测试 `__tests__/record/l1-doc-type-filter.test.ts`（含会话模式不受影响的对照） |
| atomic/search 时间范围 + scene_name 显式返回：`executeMemorySearch` 新增 `timeStartMs/timeEndMs`（按命中项 updated_at 在 RRF 合并后、截断前过滤），`handleAtomicSearch` 接线 `time_start/time_end`（请求 schema 上游本就带，此前被忽略）；`AtomicDetail`/search/query 响应统一加 `scene_name`（与 `background` 同源同值） | 记忆召回支持按时间范围圈定；调用方无需再从 `background` 别名猜场景名；回归测试 `__tests__/search/memory-search-time-scene.test.ts` |
