...
export function createL2Runner(opts: {
  ...
  return async (sessionKey: string, cursor?: string) => {
    ...
    if (vectorStore && !vectorStore.isDegraded()) {
      ...
      // fork 文档子系统（2026-08-19 定稿）：L2 场景块只消费会话派生原子。
      // 文档派生原子（source_kind='document'）不进 L2 —— L3 persona 又只从
      // 场景块生成，因此这里同时是 L3 内容侧的闸门；L3 触发计数侧的排除见
      // markL1ExtractionComplete 的 personaVisibleExtracted。
